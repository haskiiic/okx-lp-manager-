...
class LPManager:
    ...
    async def create_lp_position(
        ...
        try:
            ...
            return {
                ...
                "note": "如需持久化存储，请启动数据库服务"
                if isinstance(position_id, str) and position_id.startswith("mock_")
                else None,
            }
    ...
    async def execute_lp_transaction(
        self, position_id: int, signed_tx: str, account_id: str
    ) -> Dict[str, Any]:
        """执行LP交易
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "signed_tx_received": signed_tx[:20] + "..."
                    if len(signed_tx) > 20
                    else signed_tx,
                    "account_id": account_id,
                }
    ...
    async def get_lp_positions(self, wallet_address: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 方法1: 优先从区块链直接获取真实数据
            try:
                real_positions = await self.blockchain_reader.get_real_lp_positions(
                    wallet_address, "bsc"
                )
                if real_positions:
    ...
    async def _sync_positions_to_db(
        self, wallet_address: str, real_positions: List[Dict[str, Any]]
    ):
        """将区块链数据同步到数据库"""
        try:
            async with AsyncSessionLocal() as db:
    ...
    async def monitor_positions(self):
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                result = await db.execute(
                    select(LPPosition).where(
                        LPPosition.status.in_(["active", "pending_confirmation"])
                    )
                )
    ...
    async def _monitor_single_position(self, position: LPPosition, db: AsyncSession):
        ...
        try:
            if position.position_id:
                ...
                # 检查是否需要重平衡
                needs_rebalance = await self._check_rebalance_needed(
    ...
    async def _check_rebalance_needed(
        self, position: LPPosition, position_info: Dict
    ) -> bool:
        """检查是否需要重平衡"""
        # 简化的重平衡检查逻辑
        # 实际应用中需要更复杂的算法
    ...
    async def _check_wallet_balance(
        ...
        try:
            ...
            try:
                ...
                balance0 = float(
                    balance0_result.get("data", [{}])[0].get("tokenBalance", "0")
                )
            except Exception as e:
            ...
            try:
                ...
                balance1 = float(
                    balance1_result.get("data", [{}])[0].get("tokenBalance", "0")
                )
            except Exception as e:
            ...
            return {
                ...
                "message": "余额充足"
                if sufficient
                else f"余额不足: 需要 {amount0_needed}, 有 {balance0}",
    ...
    def _get_chain_index(self, network: str) -> str:
        """获取链索引"""
        chain_indexes = {"ethereum": "1", "bsc": "56", "polygon": "137"}
        return chain_indexes.get(network, "56")

    async def create_multiple_lp_positions(
        self, positions_config: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """批量创建LP头寸
    ...
    async def start_auto_rebalance(
        self, wallet_address: str, rebalance_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """启动指定钱包的自动重平衡
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                if not wallet:
                    # 创建钱包记录
                    wallet = Wallet(
                        address=wallet_address, created_at=datetime.utcnow()
                    )
                    db.add(wallet)
                    await db.flush()
                ...
                for position in positions:
                    await db.execute(
                        ...
                        .where(LPPosition.id == position.id)
                        .values(
                            auto_rebalance_enabled=rebalance_config.get(
                                "enabled", True
                            ),
                            rebalance_threshold=rebalance_config.get(
